...
Sentinel-1 (VV/VH/разность). Точки строятся через локальную
азимутальную равнопромежуточную проекцию (AEQD) с центром в центре
кратера — в этой проекции расстояние и азимут от центра сохраняются
точно, без приближённого пересчёта градусов в метры.
